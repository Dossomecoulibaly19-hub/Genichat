...
app = Flask(__name__)
app.secret_key = "genie_v33_whatsapp_final"
...
.avatar-big {width:150px; height:150px; border-radius:50%; margin:0 auto 20px; display:flex; align-items:center; justify-content:center; background:#2A3942; font-size:60px; font-weight:bold; border:4px solid #00A884; background-size:cover; background-position:center; cursor:pointer; overflow:hidden; position:relative;}
...
.crop-area {position:relative; width:100%; height:400px; display:flex; justify-content:center; align-items:center; overflow:hidden; touch-action:none;}
.crop-circle {position:absolute; width:200px; height:200px; border:4px solid #00A884; border-radius:50%; box-shadow:0 0 0 9999px rgba(0,0,0,0.8); pointer-events:none; z-index:2;}
.crop-img {position:absolute; max-width:none; max-height:none; cursor:grab; z-index:1;}
...
<form method="POST" action="/register" id="regForm"><input type="hidden" id="photo_data" name="photo_data">
...
const cropImg = document.getElementById('cropImg'); const preview = document.getElementById('preview'); const cropArea = document.querySelector('.crop-area');
document.getElementById('photo').onchange = function(e){const file=e.target.files[0]; if(!file) return;const reader=new FileReader();reader.onload=function(ev){cropImg.src = ev.target.result;document.getElementById('cropModal').style.display='flex';scale=1; posX=0; posY=0; updateTransform();}reader.readAsDataURL(file);}
...
document.onpointermove = document.ontouchmove = e=>{if(!isDragging) return; const p=getPointerPos(e); posX = p.clientX - startX; posY = p.clientY - startY; updateTransform();}
document.onpointerup = document.ontouchend = ()=>{isDragging=false;}
function zoom(v){scale+=v; if(scale<0.5)scale=0.5; if(scale>4)scale=4; updateTransform();}
...
const imgWidth = cropImg.naturalWidth * scale; const imgHeight = cropImg.naturalHeight * scale;
ctx.drawImage(cropImg, 75 - imgWidth/2 + posX, 75 - imgHeight/2 + posY, imgWidth, imgHeight);
...
<form method="POST" action="/update_profile" id="setForm"><input type="hidden" id="photo_data" name="photo_data" value="{{ photo }}">
...
function renderContacts(data){
    let html=''; for(let c of data.contacts){
        ...
        <div style="flex:1;"><b>${data.users[c].nom}</b><br><small>${c}</small></div>
...
loadAll();
fetch('/get_msg/'+AMI_CODE).then(r=>r.json()).then(serverMsgs=>{saveLocal(serverMsgs); loadAll();});
